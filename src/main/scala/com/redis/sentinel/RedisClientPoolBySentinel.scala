...
class RedisClientPoolBySentinel(val masterName: String, val sentinelCluster: SentinelCluster, val maxIdle: Int = 8, val database: Int = 0, val secret: Option[Any] = None, poolConfig: RedisClientPoolConfig, poolListener: Option[PoolListener]) extends RedisClientPool with Log
  ...
  private def init {
    ...
    pool = new RedisClientPoolByAddress(redisNode.copy(name = redisNode.host + ":" + String.valueOf(redisNode.port), maxIdle = maxIdle, database = database, secret = secret), poolConfig, poolListener)
  ...
  private def updatePool(redisNode: RedisNode){
    ...
    val newPool = new RedisClientPoolByAddress(redisNode.copy(name = redisNode.host + ":" + String.valueOf(redisNode.port), maxIdle = maxIdle, database = database, secret = secret), poolConfig)
    val oldPool = pool
    pool = newPool
    oldPool.close
  }
